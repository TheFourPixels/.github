...
- **Стилистика:** цвета `#FFDD2D`, `#121212`, `#FFFFFF`; шрифт `Tinkoff Sans`.  
...
### 💳 Транзакции
- Импорт из псевдо-банка (`/transactions?month=YYYY-MM`, JSON)  
- Идемпотентность 
- Автоклассификация (merchant > MCC > keyword) *(следующий семестр)*
...
| Сущность | Описание |
|-----------|-----------|
| `User` | Пользователь, email, пароль (bcrypt) |
...
| **Безопасность** | JWT, HTTPS, bcrypt, rate limit 10 req/s |
| **Надёжность** | Транзакции в БД, ежедневный backup |
| **Масштабируемость** | Модульная архитектура, Docker |
| **Логирование** | JSON (info, warn, error) через Winston |
...
- ≥ 80% автоклассифицированных транзакций  
- +20% вовлечённости пользователей (MAU/DAU)  
